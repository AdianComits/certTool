...
$OPENSSL  x509    -req -in ee_LDevID_req1_serv.pem -CA root_LDevID.pem -CAkey root_LDevID_key.key \
...
CN="10.10.10.1" ON="Device Manufacturer 1" $OPENSSL req -config ca_ext.cnf\
 ...
 #Sign request: end entity extensions
$OPENSSL  x509    -req -in ee_LDevID_req2_serv.pem -CA root_LDevID.pem -CAkey root_LDevID_key.key \
	   -extfile ca_ext.cnf -extensions usr_cert_san_server   -out ee_LDevID2_server.crt
...
$OPENSSL ecparam -name prime256v1  -genkey -out ee_LDevID_key3_serv.key
 ...
 #Sign request: end entity extensions
$OPENSSL  x509  -req -in ee_LDevID_req3_serv.pem -CA root_LDevID.pem -CAkey root_LDevID_key.key \
