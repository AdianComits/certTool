...
$OPENSSL ecparam -name prime256v1  -genkey -out root_hersteller_IDev_key.key

 CN="IDev Root CA" ON="Device Manufacturer 1" $OPENSSL req -new -config ca.cnf  \
	-key root_hersteller_IDev_key.key  -sha256 -out root_hersteller_IDev_csr.csr

$OPENSSL x509 -extfile ca.cnf -extensions v3_ca  -req -sha256 -in root_hersteller_IDev_csr.csr -signkey root_hersteller_IDev_key.key -out root_hersteller_IDev.crt

# End Entity certificate: create request first
$OPENSSL ecparam -name prime256v1  -genkey -out ee_hersteller_key.key

$OPENSSL req -subj '/CN=12345abcd' \
	-new -sha256  -key ee_hersteller_key.key -out ee_hersteller_req.csr

 #Sign request: end entity extensions
 $OPENSSL  x509 -req -in ee_hersteller_req.csr -CA root_hersteller_IDev.crt -CAkey root_hersteller_IDev_key.key \
    -CAcreateserial -extfile ca.cnf -extensions usr_cert   -out ee_hersteller.crt
