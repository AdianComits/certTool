...
$OPENSSL ecparam -name prime256v1  -genkey -out ee_LDevID_key_serv.key
 ...
 #Sign request: end entity extensions
$OPENSSL  x509  -startdate 19900101000000Z -enddate 99991231235959Z -req -in ee_LDevID_req_serv.pem -CA root_LDevID.pem -CAkey root_LDevID_key.key \
...
$OPENSSL ecparam -name prime256v1  -genkey -out ee_LDevID_key_test.key
 ...
 CN="test.com" ON="Device Manufacturer 1" $OPENSSL req -config ca_ext.cnf\
	-new -sha384  -key ee_LDevID_key_test.key -out ee_LDevID_req_test_san.pem

 #Sign request: end entity extensions
$OPENSSL  x509  -startdate 19900101000000Z -enddate 99991231235959Z -req -in ee_LDevID_req_test_san.pem -CA root_LDevID.pem -CAkey root_LDevID_key.key \
	   -extfile ca_ext.cnf -extensions usr_cert_san_server   -out ee_LDevID_server_san.crt
